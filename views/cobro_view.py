...
def cobro_view(page: ft.Page, carrito, on_venta_completada, cliente_id=None, cliente_nombre="Sin cliente"):
    ...
    lbl_cambio = ft.Text("$0.00", size=22, weight=ft.FontWeight.BOLD, color=COLOR_ACENTO)
    ...
    for pid, item in carrito.items.items():
        ...
        filas_orden.append(
            ft.Container(
                content=ft.Row(
                    controls=[
                        ft.Text(item["nombre"], size=13, color=COLOR_TEXTO, expand=True, max_lines=1, overflow=ft.TextOverflow.ELLIPSIS),
                        ft.Text(f"x{item['cantidad']}", size=12, color=COLOR_SUBTEXTO, width=30),
                        ft.Text(f"${sub:.2f}", size=13, color=COLOR_TEXTO, weight=ft.FontWeight.BOLD, width=65, text_align=ft.TextAlign.RIGHT),
                    ],
                ),
                bgcolor=COLOR_FONDO,
                border_radius=8,
    ...
    # ── Botones método pago ────────────────────────────────
    btn_efectivo = ft.ElevatedButton("💵 Efectivo", bgcolor=COLOR_ACENTO, color="white", expand=True, height=48, on_click=lambda e: seleccionar_metodo("efectivo"))
    btn_tarjeta = ft.ElevatedButton("💳 Tarjeta", bgcolor="#e0e0e0", color=COLOR_TEXTO, expand=True, height=48, on_click=lambda e: seleccionar_metodo("tarjeta"))
    ...
    def seleccionar_metodo(metodo):
        ...
        if metodo == "efectivo":
            ...
            # Limpiamos estados de error al cambiar a tarjeta
            lbl_estado.value = ""
        page.update()
    ...
    def generar_ticket_pdf(venta_id):
        try:
            ...
            pdf.cell(0, 6, datetime.now().strftime("%d/%m/%Y %H:%M:%S"), ln=True, align="C")
            ...
            for pid, item in carrito.items.items():
                ...
                pdf.cell(25, 7, str(item["cantidad"]), border=0, align="C")
                pdf.cell(35, 7, f"${item['precio']:.2f}", border=0, align="R")
            ...
            pdf.set_font("Helvetica", "", 10)
            pdf.cell(150, 7, f"Método: {metodo_pago['valor'].capitalize()}", border=0, align="R")
            pdf.ln()

            if metodo_pago["valor"] == "efectivo":
                pdf.cell(150, 7, f"Recibido: ${monto_recibido['valor']:.2f}", border=0, align="R")
                pdf.ln()
                cambio = monto_recibido["valor"] - total
                pdf.set_font("Helvetica", "B", 10)
                pdf.cell(150, 7, f"Cambio: ${cambio:.2f}", border=0, align="R")
                pdf.ln()

            pdf.ln(6)
            pdf.set_font("Helvetica", "I", 9)
            pdf.cell(0, 6, "¡Gracias por su compra!", ln=True, align="C")
    ...
    def confirmar_venta(e):
        ...
        try:
            # CAMBIO 2: Corrección de nulos para pago con tarjeta
            if metodo_pago["valor"] == "efectivo":
                monto_r  = monto_recibido["valor"]
                cambio_r = monto_recibido["valor"] - total
            else:
                monto_r  = total # En tarjeta se cobra exacto
                cambio_r = 0.0
            ...
            def abrir_pdf(e):
                if ruta_pdf:
                    import subprocess
                    subprocess.Popen(["start", "", os.path.abspath(ruta_pdf)], shell=True)
                else:
                    page.snack_bar = ft.SnackBar(content=ft.Text("❌ El PDF no se generó. Verifica que fpdf2 esté instalado.", color="white"), bgcolor=COLOR_ROJO)
                    page.snack_bar.open = True
                    page.update()

            dialogo_ok = ft.AlertDialog(
                modal=True,
                title=ft.Text("✅ Venta Completada", weight=ft.FontWeight.BOLD),
                content=ft.Column(
                    controls=[
                        ft.Text(f"Venta #{venta_id} registrada", size=14, color=COLOR_TEXTO),
                        ft.Text(f"Total: ${total:.2f}", size=13, color=COLOR_ACENTO, weight=ft.FontWeight.BOLD),
                        ft.Text(
                ...
                actions=[
                    ft.TextButton("Nueva Venta", on_click=cerrar_y_volver),
                    ft.ElevatedButton("📄 Ver Ticket", bgcolor=COLOR_AZUL, color="white", on_click=abrir_pdf),
                ],
            ...
            page.snack_bar = ft.SnackBar(content=ft.Text(f"❌ Error al cobrar: {ex}", color="white"), bgcolor=COLOR_ROJO)
            page.snack_bar.open = True
            page.update()
    ...
    return ft.Column(
        controls=[
            ft.Row(
                controls=[
                    ft.Text("💳 Cobro", size=22, weight=ft.FontWeight.BOLD, color=COLOR_TEXTO),
                    ft.Container(expand=True),
                    ft.TextButton("← Regresar", on_click=lambda e: on_venta_completada(), style=ft.ButtonStyle(color=COLOR_SUBTEXTO))
                ]
            ),
                    ...
                    ft.Container(
                        content=ft.Column(
                            controls=[
                                ...
                                ft.Row(
                                    controls=[
                                        ft.Text("TOTAL", size=16, weight=ft.FontWeight.BOLD, color=COLOR_TEXTO),
                                        ft.Text(f"${total:.2f}", size=22, weight=ft.FontWeight.BOLD, color=COLOR_ACENTO)
                                    ],
                                    alignment=ft.MainAxisAlignment.SPACE_BETWEEN
                                )
                            ],
                        ...
                        expand=3, bgcolor=COLOR_TARJETA, border_radius=14, padding=16,
                        shadow=ft.BoxShadow(blur_radius=8, color="#00000010", offset=ft.Offset(0, 2))
                    ),
                    ft.Container(
                        content=ft.Column(
                            controls=[
                                ft.Text("💰 Método de Pago", size=15, weight=ft.FontWeight.BOLD, color=COLOR_TEXTO),
                                ft.Divider(),
                                ft.Row(controls=[btn_efectivo, btn_tarjeta], spacing=10),
                                ft.Container(height=8),
                                contenedor_efectivo,
                                ft.Container(expand=True),
                                ft.ElevatedButton(
                                    "✅  CONFIRMAR VENTA", bgcolor=COLOR_ACENTO, color="white", height=55, expand=True, on_click=confirmar_venta,
                                    style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=10))
                                )
                            ],
                            spacing=8, expand=True
                        ),
                        expand=2, bgcolor=COLOR_TARJETA, border_radius=14, padding=16,
                        shadow=ft.BoxShadow(blur_radius=8, color="#00000010", offset=ft.Offset(0, 2))
                    )
                ],
